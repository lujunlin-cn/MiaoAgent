...
| 痛点 | MiaoAgent 方案 |
|------|---------------|
| 情感数据上云的隐私焦虑 | 全栈端侧部署，零云依赖，NemoClaw 沙箱网络隔离 |
...
> 以下数据由 `run_all_tests.py` 自动化脚本在 DGX Spark 上实测生成
...
整个智能体运行在 NemoClaw 沙箱中。`sandbox-policy.yaml` 仅放行 Phi-4 裁判直连端口（`:8356`），Qwen3 通过 OpenShell 推理路由代理，所有其他外部网络请求均被内核级拦截。
...
| 沙箱 | NemoClaw / OpenShell 0.1.0 | 沙箱运行时与推理隐私路由 |
...
MiaoAgent 运行在 NemoClaw 沙箱中，确保所有推理和感知数据零外联。

两个推理模型采用不同通信路径：
- **Qwen3-30B**（对话核心）→ OpenShell `inference.local` 推理路由，由 gateway 代理转发
- **Phi-4-multimodal**（裁判引擎）→ `sandbox-policy.yaml` 网络白名单，直连 `:8356`

```bash
# 首次部署
sudo nemoclaw onboard
sudo nemoclaw policy apply sandbox-policy.yaml

# 注册 Qwen3 推理路由（Phi-4 走白名单，不需要注册）
...
# 重启后恢复（NemoClaw alpha 已知限制，不支持自动恢复）
sudo bash nemoclaw_recover.sh
```

> ⚠️ DGX Spark 关机/重启后 NemoClaw 沙箱需要手动恢复（Issue #159, #768）。`nemoclaw_recover.sh` 自动完成 k3s 临时停止、gateway 重启、Qwen3 推理路由重注册、k3s 恢复等步骤。

### 7. 启动 MiaoAgent

```bash
# 连接沙箱
sudo nemoclaw <sandbox名字> connect
...
> **开发调试**：沙箱外直连 TRT-LLM 开发时，无需第 6 步，直接运行 `python3 frontend/web_ui.py`（默认 `ENGINE=trtllm`）。
